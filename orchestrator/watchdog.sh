...
# watchdog.sh — Keeps gaze daemon alive after OOM kills
# Install: replace ~/.termux/boot/startup.sh with this script
# The gaze daemon handles everything startup.sh used to do:
...
while true; do
  ...
  if daemon_alive; then
    ...
    echo "[$(date)] Starting gaze boot..." >> "$LOG_DIR/watchdog.log"
    ...
    if [ $EXIT_CODE -ne 0 ]; then
      echo "[$(date)] gaze boot failed (code=$EXIT_CODE), retrying in 5s..." >> "$LOG_DIR/watchdog.log"
      sleep 5
      continue
    fi
